...
function calculatePrice(mapping, percent) {
    if (percent === 1) {
        return mapping;
    };
    _.each(mapping, function(instance) {
        console.log('instance:', instance, 'instance$$', instance['price']);
        var originalPrice = instance['price'];
        instance['price'] = originalPrice * percent;
    });
    return mapping
};